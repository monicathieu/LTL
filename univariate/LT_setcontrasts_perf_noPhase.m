...
if ~strcmp(analysis, 'analysisByPerf_noPhase')
    error('we can''t help you yet!, please try again when there are more analysis options (OR FIX THE CODE, DUMMY!)');
end

thisAnalysisDir = fullfile(par.analysisdir, 'analysisByPerf_noPhase');
cd(thisAnalysisDir);
...
con.hitSrcCor_gr_hitSrcInc = idx.bf1 .* balCon_SimpFx(idx.hitSrcCor - idx.hitSrcInc);
con.hitSrcCor_gr_NoSrcHit = idx.bf1 .* balCon_SimpFx(idx.hitSrcCor - idx.hitNoSrc);
