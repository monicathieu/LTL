function [subj] = LTC_mvpa_load_and_preprocess_raw_data_AG(S)
...
elseif strcmp(S.patternType, 'raw')
    ...
    if S.denoise
        ...
        subj = init_subj(S.exp_name,S.subj_id);
        subj = load_spm_mask(subj,S.roi_name,S.roi_file);
        subj = load_analyze_pattern(subj,'epi',S.roi_name, S.img_files,'single',true);
        ...
        % hp filter the data
        subj = hpfilter_runs(subj,'epi','runs',100,2);
        subj = remove_mat(subj,'pattern','epi');
        
        % zscore the data
        subj = zscore_runs(subj,'epi_hp','runs');
        subj = remove_mat(subj,'pattern','epi_hp');
    end
