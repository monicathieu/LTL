function [res, results]= MMT_run_mvpa_general(subj_array, task, saveName, portion)
%function [res, results] = LT_run_mvpa_general(subj_array, task, saveName, portion)

for subNum=(1:length(subj_array))
    
    %% load general parameter information
    tic;
    [S idxTr idxTe par] = MMT_mvpa_params(subj_array(subNum), task);
    if nargin > 3 % if specified then set it
        S.portion = portion;
    else
        S.portion = []; 
    end
    ...
    %% Onsets
    S = PM_mvpa_onsets_and_images(S); % find this function
    ...
    if (S.use_premade_workspace&&existWorkspace) % if we are supposed to use premade workspace, and one with the correct name exists
        ...
        preprocFunct = str2func(S.loadAndPreprocFunctName)
        [subj] = preprocFunct(S);
    ...
    for n = 1: S.num_results_iter
        ...
        if strcmp(S.patternType, 'raw')
            ...
            if strcmp(S.trainTask, S.testTask)
                ...
                meta_runs_test = [];
                randomNFold = ceil(AG_shuffle(1:length(meta_runs_condensed))/(length(meta_runs_condensed)/S.nFolds));
...
%             end
        ...
        %store results
        res.subj{subNum}.penalty(1).nVox(1).weights(1).iter{n} = results;
        res.subj{subNum}.penalty(1).nVox(1).weights(1).S = S;
